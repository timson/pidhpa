...
```sh
make deploy IMG=<some-registry>/pidhpa:tag
```
...
```sh
make build-installer IMG=<some-registry>/pidhpa:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/pidhpa/<tag or branch>/dist/install.yaml
```
